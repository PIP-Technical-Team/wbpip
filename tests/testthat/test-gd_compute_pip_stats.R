...
test_that('gd_compute_pip_stats() returns correct results', {
  ...
  res <- gd_compute_pip_stats(
    ...
    requested_mean = 88.56682, povline = 1.9)
  expect_equal(res$mean, 88.56682)
  expect_equal(res$median, 69.67302, tolerance = 1.5e-06)
  ...
  skip('poverty stats are incorrect (all values are 0)')
  expect_equal(res$headcount, 0.3713033)
