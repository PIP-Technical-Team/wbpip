...
dl <- lapply(dl, function(df) {
  md_clean_data(df, welfare = 'welfare', weight = 'weight')$data
})
```
...
These survey means might look like they very large, but that is because the underlying welfare values are reported in yearly values. Since international poverty statistics typically are reported at daily levels, it might make more sense to convert them to daily values.
...
We thus create a new vector, `svy_mean_ppp,` by using the function `deflate_welfare_mean()` to adjust the values. Here we imagine a PPP value of 1081, a CPI value for the first survey year of 0.6 and a CPI value for the second survey year of 0.9.
...
This gives us a a daily welfare mean of \$`r round(svy_mean_ppp[1], 1)` in 2000 and a welfare mean of \$`r round(svy_mean_ppp[2], 1)` in 2010.

## Calculate the predicted mean for the request year

We are now almost ready to calculate poverty statistics for the request year we are interested in, but there is one final step we need to conduct first. We need to predict what the two survey means would have been in the request year.

This can been done by calculating the difference in welfare for the two respective survey years and the request year. Since we don't know the value for the request year we need a set of proxy values to calculate the growth rate. Typically national accounts data, e.g. the Gross Domestic Product (GDP) or Household Final Consumption Expenditure (HFCE), is used for this purpose.
